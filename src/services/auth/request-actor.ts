...
import { requireAdminEmployee, requireEnabledEmployee } from "@/services/auth/employee-store";
...
export async function actorFromRequest(request: Request): Promise<RequirementActor | undefined> {
  ...
  if (session) await requireEnabledEmployee(session.openId);
  return session ? { id: session.openId, name: session.name } : undefined;
}

/** Browser writes require a Feishu session and an explicit publisher allowlist. */
export async function publisherFromRequest(request: Request): Promise<RequirementActor> {
  ...
  const publishers = (process.env.FEISHU_PUBLISHER_OPEN_IDS ?? "")
    .split(",")
    .map((item) => item.trim())
    .filter(Boolean);
  if (!publishers.includes(actor.id)) throw new Error("当前飞书账号没有发布权限。");
  return actor;
}
