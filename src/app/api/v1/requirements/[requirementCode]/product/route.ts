...
import { setRequirementProduct } from "@/services/requirement/repository";
...
export async function PATCH(request: Request, { params }: { params: Promise<{ requirementCode: string }> }) {
  try {
    ...
    return apiJson(await setRequirementProduct((await params).requirementCode, body.productId));
