...
type ModelForm = { name: string; baseUrl: string; model: string; apiKey: string; isDefault: boolean };
const emptyForm: ModelForm = { name: "", baseUrl: "", model: "", apiKey: "", isDefault: true };
...
export function ModelManager() {
  ...
  function beginEdit(item: ModelSummary) {
    setEditing(item);
    setForm({ name: item.name, baseUrl: item.baseUrl, model: item.model, apiKey: "", isDefault: item.isDefault });
    setError("");
    setFormOpen(true);
  }
    ...
    try {
      if (editing) {
        await request<ModelSummary>("/api/v1/models", {
          method: "PATCH",
          body: JSON.stringify({ id: editing.id, name: form.name, baseUrl: form.baseUrl, model: form.model, apiKey: form.apiKey || undefined, isDefault: form.isDefault }),
        });
      } else {
