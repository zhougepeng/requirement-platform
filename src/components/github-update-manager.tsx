...
export function GithubUpdateManager({ initialOpen = false, hideTrigger = false }: { initialOpen?: boolean; hideTrigger?: boolean }) {
  ...
  return <>
    ...
    {open ? <div className="model-manager-layer" role="presentation"><button className="model-manager-backdrop" aria-label="关闭系统更新" onClick={() => setOpen(false)} /><section className="model-manager-dialog github-update-dialog" role="dialog" aria-modal="true" aria-labelledby="github-update-title">
      <header><div><h2 id="github-update-title">系统更新</h2><p>检查需求库 GitHub 仓库的 main 分支。</p></div><button className="model-manager-close" onClick={() => setOpen(false)} aria-label="关闭系统更新"><Icon name="close" /></button></header>
      ...
      <footer className="github-update-footer"><button className="model-cancel" onClick={() => setOpen(false)}>关闭</button><button className="model-cancel" disabled={loading || pulling} onClick={() => void check()}><Icon name="refresh" />重新检查</button>{status?.canPull && !complete ? <button className="model-save" disabled={pulling} onClick={() => void pull()}>{pulling ? "拉取中…" : "拉取更新"}</button> : null}</footer>
    </section></div> : null}
  </>;
}
