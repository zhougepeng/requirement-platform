...
type MaterialScope = "project" | "public" | "pm_skill";
...
const SCOPE_LABELS: Record<string, string> = { project: "项目资料", public: "公共资料", pm_skill: "产品经理经验", product: "产品规范", global: "公共规范" };

// 产品规范特征：命中说明这条更像“产品该长什么样”，而不是“我该怎么做”。只做提示，不阻断保存。
const PRODUCT_SPEC_SIGNAL = /#[0-9a-fA-F]{3,8}\b|\d+\s*(?:px|rpx|rem)\b|圆角|字号|字重|行高|色值|主色|辅色|阴影|边框|--[a-z][\w-]*\s*:|组件命名|命名规范|(?:所有|全部)(?:页面|组件|按钮|弹窗)?(?:必须|都要|一律)(?:统一|一致)|统一(?:采用|使用|为)/;

function productSpecSignal(value: string) {
  return PRODUCT_SPEC_SIGNAL.exec(value)?.[0] ?? "";
}
...
function ProductSpecView({ product, spec, snapshots = EMPTY_SPEC_SNAPSHOTS, canEdit, onRestore }: { product: Product; spec: ProductSpec | null; snapshots?: ProductSpecSnapshot[]; canEdit?: boolean; onRestore?: (snapshot: ProductSpecSnapshot) => void }) {
  ...
  return <article className="material-preview material-product-spec-view">
    ...
    <section className="material-spec-section"><header><div><h4>Demo 规范</h4><p>用于约束页面结构、交互状态和演示完成度。</p></div>{spec.demoBaseline ? <span className="material-spec-count">基线扩展模式</span> : null}</header><div className="material-spec-rule-grid"><SpecRuleGroup title="布局原则" values={spec.demo.layoutPrinciples} /><SpecRuleGroup title="组件复用" values={spec.demo.componentReuseRules} /><SpecRuleGroup title="交互要求" values={spec.demo.interactionRequirements} /><SpecRuleGroup title="开发约束" values={spec.demo.constraints} /></div>{spec.demoBaseline ? <div className="material-spec-baseline"><b>原始 Demo 基线</b><span>来源需求：{spec.demoBaseline.sourceRequirementCode} · V{spec.demoBaseline.sourceVersionNo}</span><span>页面 {spec.demoBaseline.pageIds.length} 个 · 关键交互标识 {spec.demoBaseline.requiredDemoIds.length} 个</span><code>{spec.demoBaseline.sourceHash.slice(0, 12)}…</code><details><summary>查看关键标识</summary><div className="material-spec-chip-row">{spec.demoBaseline.requiredDemoIds.map((id) => <span key={id}>{id}</span>)}</div></details></div> : null}</section>
...
export function MaterialLibrary({ projects, canEdit }: { projects: Project[]; canEdit?: boolean }) {
  ...
  async function save() {
    ...
    if (target.scope === "pm_skill") {
      const signal = productSpecSignal(content);
      if (signal && !window.confirm(`这条内容包含产品规范特征（“${signal}”），更像是“产品该长什么样”。\n经验库说“我该怎么做”，产品规范说“产品该长什么样”。\n仍然保存到经验库？`)) return;
    }
  ...
  const pmSkillDirectories = directories.filter((directory) => directory.scope === "pm_skill");

  return <section className="material-library">
    <header className="material-library-head"><div><h1>资料库</h1><p>项目资料与公共资料会自动作为需求智能体的补充知识；产品经理经验库会在生成 PRD / Demo 时作为“怎么做”的经验带入。</p></div></header>
    ...
    <div className="material-library-layout">
      <aside className="material-tree">
        ...
        <div className="material-tree-group-head"><small>产品经理经验库</small>{canEdit ? <button className="material-tree-add" onClick={() => openCreateDirectory({ scope: "pm_skill", label: "产品经理经验库" })} title="新建经验目录" aria-label="新建经验目录"><Icon name="plus" /></button> : null}</div>
        <div className="material-tree-list">{pmSkillDirectories.filter((directory) => !directory.parentId).map((directory) => <DirectoryBranch key={directory.id} directory={directory} directories={pmSkillDirectories} target={target} canEdit={canEdit} depth={0} onChoose={choose} onCreate={openCreateDirectory} />)}</div>
